...
export function EditorStructurePanel({
  ...
  return (
    <aside className="flex h-full w-[320px] max-w-[34vw] flex-col border-r border-slate-800 bg-slate-950 text-slate-100">
      <header className="border-b border-slate-800 px-4 py-3">
        <div className="flex items-center gap-2 text-sm font-semibold text-sky-300">
          <Building2 className="size-4" aria-hidden="true" />
          Structure
        </div>
        <h2 className="mt-1 truncate text-base font-semibold text-white">{project?.name ?? 'Loading project...'}</h2>
      </header>
      ...
      <section className="border-b border-slate-800 px-4 py-3">
        ...
        <div className="space-y-1">
          <button
            type="button"
            className="mb-1 w-full rounded-md border border-sky-500/50 bg-sky-500/20 px-2 py-1.5 text-left text-xs font-semibold text-sky-100 transition hover:bg-sky-500/30 disabled:opacity-50"
          ...
          {floors.length === 0 ? (
            <div className="rounded-md border border-slate-800 bg-slate-900 px-2 py-1.5 text-xs text-slate-400">No floors</div>
          ) : (
              ...
              return (
                <div
                  ...
                  className={`rounded-md border px-2 py-1.5 text-sm ${
                    isActive ? 'border-sky-500/60 bg-sky-500/20' : 'border-slate-800 bg-slate-900'
                  }`}
                >
                  {isEditing ? (
                    <div className="space-y-1.5">
                      ...
                      <div className="flex gap-1">
                        <button
                          type="button"
                          className="rounded border border-sky-500/50 bg-sky-500/20 px-2 py-0.5 text-xs text-sky-100"
                    ...
                    <>
                      <button
                        ...
                        className={`flex w-full items-center justify-between text-left transition ${
                          isActive ? 'text-sky-100' : 'text-slate-300 hover:text-white'
                        }`}
                        onClick={() => onSelectFloor(floor.id)}
                      >
                        <span>{floor.label}</span>
                        <span className="text-xs text-slate-400">{floor.code}</span>
                      </button>
                      <div className="mt-1 flex gap-1">
                        <button
                          type="button"
                          className="rounded border border-slate-700 px-2 py-0.5 text-xs text-slate-300 hover:text-white"
                          onClick={() => {
                        ...
                        <button
                          type="button"
                          className="rounded border border-rose-500/40 px-2 py-0.5 text-xs text-rose-200 hover:bg-rose-500/10"
                          disabled={isUpdatingFloor}
      ...
      <section className="min-h-0 flex-1 p-4">
        ...
        <div className="relative mb-2">
          ...
          <input
            ...
            className="w-full rounded-md border border-slate-700 bg-slate-900 py-2 pl-8 pr-2 text-sm text-slate-100 outline-none ring-sky-400 focus:ring"
          />
        </div>
        <div className="h-full space-y-1 overflow-y-auto rounded-lg border border-slate-800 bg-slate-900/70 p-2">
          {filteredFeatures.length === 0 ? (
            <div className="px-2 py-3 text-xs text-slate-400">No objects in this view</div>
          ) : (
            filteredFeatures.map((feature) => {
              ...
              return (
                ...
                >
                  <div className="truncate text-sm font-medium">{featureTitle(feature)}</div>
                  <div className="mt-0.5 truncate text-xs text-slate-400">{tag || 'No tag'} · {feature.geometry.type}</div>
                </button>
              )
            })
          )}
