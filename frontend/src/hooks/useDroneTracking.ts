...
const trackingRouteSourceId = 'drone-tracking-route-source'
const trackingRouteLayerId = 'drone-tracking-route-layer'
...
function removeLayerSafe(map: Map, layerId: string) {
  ...
  try {
    ...
    // MapLibre may clear style during unmount; cleanup should stay quiet.
  }
}
...
export function useDroneTracking({
  ...
  const setRouteData = useCallback((points: TrackingPoint[], source: TrackingSource, droneId: string) => {
    ...
    const sourceRef = getSourceSafe(map, trackingRouteSourceId)
    if (!sourceRef) {
      return
    }
  ...
  useEffect(() => {
    ...
    const ensureTrackingLayer = () => {
      ...
      if (!map.getSource(trackingRouteSourceId)) {
        map.addSource(trackingRouteSourceId, {
          type: 'geojson',
          data: toLineStringFeature([], routeRef.current.source, routeRef.current.droneId),
        })
      }
      if (!map.getLayer(trackingRouteLayerId)) {
        map.addLayer({
          id: trackingRouteLayerId,
          type: 'line',
          source: trackingRouteSourceId,
          layout: {
            'line-join': 'round',
            'line-cap': 'round',
          },
    ...
    return () => {
      ...
      stopPositionSource()
      clearMarker()
      removeLayerSafe(map, trackingRouteLayerId)
      removeSourceSafe(map, trackingRouteSourceId)
    }
